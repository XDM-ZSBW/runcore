...
import { isGmailAvailable, getRecentMessages, searchMessages, getUnreadCount, formatMessagesForContext } from "../../google/gmail.js";
...
export const emailContextProvider: ContextProviderCapability = {
  ...
  shouldInject(message: string): boolean {
    if (!isGmailAvailable()) return false;
  ...
  async getContext(message: string): Promise<ContextInjection | null> {
    ...
    if (fromMatch) {
      const searchResult = await searchMessages(`from:${fromMatch[1]}`, 5);
      ...
      return {
        ...
        content: [
          ...
          formatMessagesForContext(searchResult.messages),
          `--- End email results ---`,
        ].join("\n"),
      };
    }
    ...
    return {
      ...
      content: [
        ...
        formatMessagesForContext(recent.slice(0, 10)),
        `--- End inbox summary ---`,
